...
void kernel_entry(void) {
    __asm__ __volatile__(
        ...
        // sscratchレジスタにある実行中プロセスのカーネルスタックのアドレスをspレジスタに設定し、同時に例外発生時のspをsscratchレジスタに保存
...
// スタックに呼び出し先保存レジスタを保存し、スタックポインタの保存・復元、そして呼び出し先保存レジスタを復元する
...
void yield(void) {
    ...
    // sscratchに実行中プロセスのカーネルスタックの初期値を設定
    __asm__ __volatile__(
